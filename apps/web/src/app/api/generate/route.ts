 ...
 * The API key never leaves the server. Mirrors the Python pipeline in apps/api/:
 *
 *   1. Validate intake payload (input_contract.md)
 *   2. Deterministic engine: fatigue state → CNS plan → volume budget → allowed exercises
 *   3. Build SYSTEM + DEVELOPER prompt (prompt_wrapper.md)
 *   4. Call claude-sonnet-4-6
 *   5. Parse + return programme JSON (output_schema.md)
 *
 * Rate limiting and Supabase caching are TODO (Month 1 Week 3).
 */
...
// In dev:       process.cwd() = apps/web/  → ../../packages/engine
// In Vercel standalone (outputFileTracingRoot = repo root):
//               process.cwd() = apps/web/  → same relative layout
...
// Loaded once at module init — cached across warm function invocations
const SPECS = {
...
// Developer context block: spec files concatenated per prompt_wrapper.md
const DEVELOPER_SPECS = [
  `### output_schema.md\n\n${SPECS.outputSchema}`,
...
// Mirror of apps/api/deus_api/engine/substitution.py INJURY_BLOCKS
const INJURY_BLOCKS: Record<string, string[]> = {
...
function blockedIdsForInjuries(injuries: string[]): Set<string> {
...
// ---------------------------------------------------------------------------
// Deterministic engine — mirrors apps/api/deus_api/engine/decision_engine.py
// ---------------------------------------------------------------------------
...
function buildPrecomputedPlan(
  ...
  // Volume budget (fatigue: reduce 30%, never reduce intensity)
  const baseVol = BASE_VOLUME[client_profile.training_age] ?? 6;
  const volumeBudget =
    fatigueState === "high"
      ? Math.max(3, Math.floor(baseVol * 0.7))
      : baseVol;

  // Dynamic CNS cap per quality_control.md and engine_instructions.md
  ...
  const n = Math.min(
    TARGET_DAYS[client_profile.training_age] ?? 4,
    eligible.length
  );
  const chosen: string[] = [];
  if (n === 1) {
  ...
  // CNS assignment: dynamic cap, no consecutive, pre-sport Low
  ...
  // Exercise pool — filter blocked by injuries
...
// ---------------------------------------------------------------------------
// Request shape (input_contract.md)
// ---------------------------------------------------------------------------
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  // Load exercise library and build precomputed plan
  let exercises: Exercise[];
  try {
    exercises = loadExercises();
  } catch {
    return NextResponse.json(
      { error: "SERVER_ERROR", message: "Exercise library unavailable" },
      { status: 500 }
    );
  }
  ...
  // Build developer context: spec files + deterministic plan
  // The LLM's only job is filling exercise slots within the plan structure.
  const planBlock =
  ...
  // Anthropic call — SYSTEM + DEVELOPER passed as system blocks per prompt_wrapper.md
  ...
  try {
    ...
    if (message.stop_reason === "refusal") {
      return NextResponse.json(
        { error: "ENGINE_REFUSED", message: "Model refused to generate programme" },
        { status: 502 }
      );
    }
